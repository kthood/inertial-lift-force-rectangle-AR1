function [forcex,forcey]=getLiftForce_AR1(x,y,rsph,U, rho, L)
%% INPUT:
...
%  (x,y) are chosen such that the particles hits the wall. 
%
%% AUTHOR
...
% otherwise proceed
    ...
    Fy = rho*U^2*rsph^4*C4y/(L^2)+rho*U^2*rsph^5*C5y/(L^3);
    Fx = rho*U^2*rsph^4*C4x/(L^2)+rho*U^2*rsph^5*C5x/(L^3); 
    locx=xloc; locy=yloc;
